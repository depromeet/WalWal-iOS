...
class AppCoordinator: CoordinatorType {
  ...
  var destination = PublishSubject<Flow>()
  var requireFromChild = PublishSubject<CoordinatorEvent<Action>>()
  var navigationController: UINavigationController
  weak var parentCoordinator: (any CoordinatorType)?
  var childCoordinator: (any CoordinatorType)?
  var baseViewController: UIViewController?
  
  required init(navigationController: UINavigationController, parentCoordinator: (any CoordinatorType)?) {
  ...
  func start() {
    ...
    let reactor = SplashReactor(
      coordinator: self,
      authUsecase: SplashUsecaseImp(
        authRepository: SplashRepositoryImp()
      )
    )
